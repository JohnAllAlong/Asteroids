...
spawnAsteroids(){
    for(let i = 0; i < this.startNum; ++i){
        ...
        this.startPosition = createVector(random(0, windowWidth), random(0, windowHeight))
...
destroyAsteroid(index){
    for(let i = 0; i < 2; ++i){
        ...
        this.asteroid = new Asteroid(this.pozish, this.heading, this.asteroids[index].sizeMult + 1)
        this.asteroids.push(this.asteroid)
    }
    this.asteroids.splice(this.asteroids[index], 1)
}
